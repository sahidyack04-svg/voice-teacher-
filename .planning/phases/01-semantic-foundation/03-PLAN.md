...
must_haves:
  truths:
    - "All key pages use semantic HTML5 elements (header, main, article, nav, footer)"
    - "Full page content present in initial HTTP response (no JS-gated content)"
    - "Heading hierarchy consistent (h1→h6, no skipped levels) on every page"
...
Purpose: Complete SEM-01 through SEM-04 on all three program pages using reusable ProgramDetail component.
